...
namespace FIK.ORM
{
    ...
    public class QueryExecutorClient
    {
        ...
        public void InsertOrUpdate<T>(object dataObject, IEnumerable<string>? insertColumns, IEnumerable<string>? updateColumns, string[]? whereColumns, string tableName = "", string schemaName = "dbo") where T : class
        {
            ...
            var _tableName = _queryBuilder.GetTableName(typeof(T), tableName, schemaName);

            string dynamicQuery;
            dynamicQuery = PrepareDynamicQuery(insertQuery, updateQuery, whereQuery, _tableName);
        ...
        private string PrepareDynamicQuery(string insertQuery, string updateQuery, string whereQuery, string _tableName)
        {
            string dynamicQuery;
            ...
            else if (_databaseProvider == DatabaseProvider.PostgreSQL)
            {
                string insertQueryLeft = insertQuery.Split(new string[] { "VALUES" }, StringSplitOptions.None)[0];
                string insertQueryRight = insertQuery.Split(new string[] { "VALUES" }, StringSplitOptions.None)[1].Trim().Trim(';', '(', ')');

                dynamicQuery = string.Format(@"
                    {2};
                    {3} SELECT {4}
                    WHERE NOT EXISTS (SELECT 1 FROM {0} {1});
                    ", _tableName, whereQuery, updateQuery, insertQueryLeft, insertQueryRight);
            }
            else
            {
                dynamicQuery = string.Format(@"
            ...
            foreach (CompositeModel c in compositeModelBuilder.GetRecordSet())
            {
               ...
               TypeDescriptor.GetProperties(c.ObjectType);
                ...
                else if (c.OperationMode == OperationMode.InsertOrUpdate)
                {
                    ...
                    dynamicQuery = PrepareDynamicQuery(insertQuery, updateQuery, whereQuery, tableName);
        ...
        private void AddCommandParameter(IDbCommand command, PropertyDescriptorCollection props, MetaDataInfo[] metaDatas, Dictionary<string, string>? whereColumns)
        {
            ...
            foreach (var metaData in metaDatas)
            {
                ...
                if (command.GetType() == typeof(Npgsql.NpgsqlCommand))
                {
                    parameter.DbType = ConvertStringToDbType(metaData.DataType);
                    parameter.Value = ConvertValueToDbType(value, parameter.DbType);
                }
                else 
                {
...
//#endif
        ...
        private DbType ConvertStringToDbType(string dataTypeString)
        {
            if (string.IsNullOrWhiteSpace(dataTypeString))
                return DbType.String;

            var normalizedType = dataTypeString.Trim().ToLowerInvariant();

            return normalizedType switch
            {
                // Numeric types
                "int" or "int4" or "integer" => DbType.Int32,
                ...
                // String types
                "varchar" or "character varying" => DbType.String,
                "char" or "character" => DbType.StringFixedLength,
                "text" => DbType.String,
                "name" => DbType.String,

                // Date/Time types
                "timestamp" or "timestamp without time zone" => DbType.DateTime,
                "timestamptz" or "timestamp with time zone" => DbType.DateTime,
                "date" => DbType.Date,
                "time" or "time without time zone" => DbType.Time,
                "timetz" or "time with time zone" => DbType.Time,
        ...
        private object? ConvertValueToDbType(object value, DbType dbType)
        {
            ...
            try
            {
                return dbType! switch
                {
                    ...
                    DbType.Date => Convert.ToDateTime(value),
                    DbType.Guid => Guid.Parse(value.ToString() ?? ""),
                    DbType.String or DbType.StringFixedLength => value.ToString(),
                    DbType.Binary => value is byte[] bytes ? bytes : Convert.FromBase64String(value.ToString() ?? ""),
                    _ => value
                };
            }
            catch
