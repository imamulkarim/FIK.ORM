        ...
        public void ExecuteCompositeModel(CompositeModelBuilder compositeModelBuilder)
        {
            ...
            foreach (CompositeModel c in compositeModelBuilder.GetRecordSet())
            {
               ...
               TypeDescriptor.GetProperties(c.ObjectType);
                ...
                {
                    var updateQuery = _queryBuilder.BuildUpdateQuery(compositeModelBuilder.GetType(), c.UpdateColumns, c.WhereColumns, c.TableName, c.SchemaName);

                    c.GeneratedQuery = updateQuery;
                }
                if (c.OperationMode == OperationMode.Delete)
                {
                    var deleteQuery = _queryBuilder.BuildDeleteQuery(compositeModelBuilder.GetType(), c.WhereColumns, c.TableName, c.SchemaName);
                ...
                else if (c.OperationMode == OperationMode.InsertOrUpdate)
                {
                    ...
                    var whereQuery = _queryBuilder.GetWhereClause(compositeModelBuilder.GetType(), c.WhereColumns, c.TableName, c.SchemaName);
